...
public class bubble_ui : MonoBehaviour
{
	...
	//don't forget to include UI --> using UnityEngine.UI;
	...
	public Sprite _spriteGameOver, _spriteYouWin;
	//0 game over, 1 you win
	// END #1
	...
	void Start ()
	{
		...
		if (resetLevelScore) {
			...
			print ("set to "+PlayerPrefs.GetInt (theMedalSTD.LevelName, 0));
		}
	}
...
//		print ("now " + _LevelScore);
		...
		if (theHeart._HeartgameOver || theTimer._isOver || isGameUIOver) {
			...
			if (theHeart._HeartgameOver || theTimer._isOver) {
				audioEnding = audio_gameOver;
				_endingGameSprite = _spriteGameOver;
			} else {
				audioEnding = audio_youWin;
				_endingGameSprite = _spriteYouWin;
			}
