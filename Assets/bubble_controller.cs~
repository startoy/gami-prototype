	...
	void bubbleSort ()
	{
		...
		if (locke) {
			...
			if (oranges [curInt].GetComponent <orangeValue> ().value > oranges [curInt + 1].GetComponent <orangeValue> ().value) {
				...
				//  
//				Vector3 cPos1 = new Vector3 (); //ตำแหน่งตัวซ้าย
//				Vector3 cPos2 = new Vector3 (); //ตำแหน่งตัวขวา
...
//						oranges [sort + 1].transform.position = oranges [sort].transform.position;
						oranges [sort + 1] = oranges [sort];

						oranges [sort] = temp;
//						oranges [sort].transform.position = temp.transform.position;
